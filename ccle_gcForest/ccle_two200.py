...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        pd.DataFrame(folds_expr).to_csv("two2/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("two2/folds_cna.csv")
        ...
        for j in range(5):
            ...
            X_test.to_csv("two2/"+str(j)+str(drug)+"_X_test_expr.csv")
            y_test.to_csv("two2/"+str(j)+str(drug)+"_y_test_expr.csv")
            ...
            X_test_cna.to_csv("two2/"+str(j)+str(drug)+"_X_test_cna.csv")
            y_test_cna.to_csv("two2/"+str(j)+str(drug)+"_y_test_cna.csv")


            ######################mgs expr part################
            levels = np.unique(np.array(y_train))
            print("levels:",levels)
            File = open("two2/"+str(j)+str(drug)+".txt", "w")
            File.write("levels:"+str(levels)+"\n")
            clf = gcForest(shape_1X=(1, 400),window=[200],stride=2,levels=levels,f=File)
            ...
            pd.DataFrame(predict_proba).to_csv("two2/"+str(j)+str(drug)+"_predict_proba.csv")
            predictions = levels[np.argmax(predict_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("two2/"+str(j)+str(drug)+"_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("two2/"+str(drug)+"five_results.csv")
        print(five_results)
        each_drug_acc.append(pd.DataFrame(five_results).iloc[:,1].mean())
        all_drug_acc.append(each_drug_acc)
    pd.DataFrame(all_drug_acc).to_csv("two2/all_drug_acc.csv")
