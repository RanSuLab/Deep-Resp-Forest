...
from GCForest_four import gcForest
import random
...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        pd.DataFrame(folds_expr).to_csv("four3/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("four3/folds_cna.csv")
        ...
        for j in range(5):
            ...
            X_test.to_csv("four3/"+str(j)+str(drug)+"_X_test_expr.csv")
            y_test.to_csv("four3/"+str(j)+str(drug)+"_y_test_expr.csv")
            ...
            X_test_cna.to_csv("four3/"+str(j)+str(drug)+"_X_test_cna.csv")
            y_test_cna.to_csv("four3/"+str(j)+str(drug)+"_y_test_cna.csv")


            ######################mgs expr part################
            levels = np.unique(np.array(y_train))
            print("levels:",levels)
            File = open("four3/"+str(j)+str(drug)+".txt", "w")
            ...
            pd.DataFrame(expr_mgs_X).to_csv("four3/expr_mgs_X.csv")
            ...
            pd.DataFrame(expr_mgs_X_test).to_csv("four3/expr_mgs_X_test.csv")
            ...
            pd.DataFrame(cna_mgs_X).to_csv("four3/cna_mgs_X.csv")
            ...
            pd.DataFrame(cna_mgs_X_test).to_csv("four3/cna_mgs_X_test.csv")
            ...
            train_predict_y = clf.cascade_forest(cna_window1,expr_window1,expr_window2,cna_window2, np.array(y_train_cna))
            ...
            #####################predict values###########################
            cascade_all_pred_prob = clf.cascade_forest(cna_window1_test,expr_window1_test,expr_window2_test,cna_window2_test)
            predict_proba = np.mean(cascade_all_pred_prob, axis=0)
            pd.DataFrame(predict_proba).to_csv("four3/"+str(j)+str(drug)+"_predict_proba.csv")
            predictions = levels[np.argmax(predict_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("four3/"+str(j)+str(drug)+"_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("four3/"+str(drug)+"five_results.csv")
        print(five_results)
        each_drug_acc.append(pd.DataFrame(five_results).iloc[:,1].mean())
        all_drug_acc.append(each_drug_acc)
    pd.DataFrame(all_drug_acc).to_csv("four3/all_drug_acc.csv")
