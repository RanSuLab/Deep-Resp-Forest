...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        pd.DataFrame(folds_expr).to_csv("four/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("four/folds_cna.csv")
        ...
        for j in range(5):
            ...
            X_test.to_csv("four/"+str(j)+str(drug)+"_X_test_expr.csv")
            y_test.to_csv("four/"+str(j)+str(drug)+"_y_test_expr.csv")
            ...
            pd.DataFrame(cna_mgs_X).to_csv("four/cna_mgs_X.csv")
            ...
            pd.DataFrame(cna_mgs_X_test).to_csv("four/cna_mgs_X_test.csv")
            ...
            train_predict_y = clf.cascade_forest(expr_window1,expr_window2,cna_window1,cna_window2, np.array(y_train_cna))
            ...
            pd.DataFrame(predict_proba).to_csv("four/"+str(j)+str(drug)+"_predict_proba.csv")
            predictions = levels[np.argmax(predict_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("four/"+str(j)+str(drug)+"_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("four/"+str(drug)+"five_results.csv")
        print(five_results)
        each_drug_acc.append(pd.DataFrame(five_results).iloc[:,1].mean())
        all_drug_acc.append(each_drug_acc)
    pd.DataFrame(all_drug_acc).to_csv("four/all_drug_acc.csv")
