...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        pd.DataFrame(folds_expr).to_csv("gctoOne/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("gctoOne/folds_cna.csv")
        ...
        for j in range(5):
            ...
            pd.DataFrame(X_test).to_csv("gctoOne/"+str(j)+str(drug)+"X_test.csv")
            pd.DataFrame(y_test).to_csv("gctoOne/"+str(j)+str(drug)+"y_test.csv")


            ######################mgs expr part################
            levels = np.unique(np.array(y_train))
            print("levels:",levels)
            File = open("gctoOne/"+str(j)+str(drug)+".txt", "w")
            File.write("levels:"+str(levels)+"\n")
            clf = gcForest(shape_1X=(1, 800),window=[100,200],stride=2,levels=levels,f=File)
            if np.shape(X_train)[0] != len(y_train):
                raise ValueError('Sizes of y and X do not match.')
            mgs_X = clf.mg_scanning(np.array(X_train), np.array(y_train))
            mgs_X_test = clf.mg_scanning(np.array(X_test))




            ######################cascade expr_cna part################
            train_predict_y = clf.cascade_forest(mgs_X, np.array(y_train))
            ...
            pd.DataFrame(predict_proba).to_csv("gctoOne/"+str(j)+str(drug)+"_predict_proba.csv")
            predictions = levels[np.argmax(predict_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("gctoOne/"+str(j)+str(drug)+"_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("gctoOne/"+str(drug)+"five_results.csv")
        print(five_results)
        each_drug_acc.append(pd.DataFrame(five_results).iloc[:,1].mean())
        all_drug_acc.append(each_drug_acc)
    pd.DataFrame(all_drug_acc).to_csv("gctoOne/all_drug_acc.csv")
