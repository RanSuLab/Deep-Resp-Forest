...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        for j in range(10):
            ...
            # X_train.to_csv(str(j)+str(drug)+"X_train_expr.csv")
            # y_train.to_csv(str(j)+str(drug)+"y_train_expr.csv")
            X_test.to_csv("single/"+str(j)+str(drug)+"_X_test_expr.csv")
            y_test.to_csv("single/"+str(j)+str(drug)+"_y_test_expr.csv")
            ...
            print("levels:",levels)
            File = open("single/"+str(j)+str(drug)+"expr.txt", "w")
            ...
            pd.DataFrame(pred_proba).to_csv("single/"+str(j)+str(drug)+"_expr_predict_proba.csv")
            # pd.DataFrame(pred_proba).to_csv(str(drug)+"_predict_proba.csv")
            predictions = clf.levels[np.argmax(pred_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("single/"+str(j)+str(drug)+"_expr_predictions.csv")
            ...
            # X_train_cna.to_csv(str(j)+str(drug)+"X_train_cna.csv")
            # y_train_cna.to_csv(str(j)+str(drug)+"y_train_cna.csv")
            X_test_cna.to_csv("single/"+str(j)+str(drug)+"_X_test_cna.csv")
            y_test_cna.to_csv("single/"+str(j)+str(drug)+"_y_test_cna.csv")

            # #
            # #
            File = open("single/"+str(j)+str(drug)+"cna.txt", "w")
            File.write("levels:"+str(levels)+"\n")
            clf = gcForest(shape_1X=(1, 400),window=[100,200],stride=2,levels=levels,f=File)
            clf.fit(np.array(X_train_cna), np.array(y_train_cna))
            pred_proba = clf.predict_proba(X=np.array(X_test_cna))
            pd.DataFrame(pred_proba).to_csv("single/"+str(j)+str(drug)+"_cna_predict_proba.csv")
            predictions = clf.levels[np.argmax(pred_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("single/"+str(j)+str(drug)+"_cna_predictions.csv")
            prediction_accuracy = accuracy_score(y_true=y_test_cna, y_pred=predictions)
            print('Layer validation accuracy = {}'.format(prediction_accuracy))
            File.write('Layer validation accuracy = {}'.format(prediction_accuracy)+"\n")
            File.close()
