...
if __name__ == '__main__':
    ...
    for i in range(15):
        ...
        pd.DataFrame(folds_expr).to_csv("two4/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("two4/folds_cna.csv")
        ...
        for j in range(5):
            ...
            X_test.to_csv("two4/"+str(j)+str(drug)+"_X_test_expr.csv")
            y_test.to_csv("two4/"+str(j)+str(drug)+"_y_test_expr.csv")
            ...
            print("levels:",levels)
            File = open("two4/"+str(j)+str(drug)+".txt", "w")
            ...
            train_predict_y = clf_expr.cascade_forest(expr_window1,expr_window2,np.array(y_train))
            if train_predict_y=="no_features":
                feature_flag = "no_features"
                print("drug"+str(drug)+":all feature importances are zeros")
                break
            cascade_all_pred_prob = clf_expr.cascade_forest(expr_window1_test,expr_window2_test)
            pred_proba = np.mean(cascade_all_pred_prob, axis=0)
            ...
            pd.DataFrame(predictions).to_csv("two4/"+str(j)+str(drug)+"_expr_predictions.csv")
            ...
            ######################cna part################
            File = open("two4/"+str(j)+str(drug)+"cna.txt", "w")
            File.write("levels:"+str(levels)+"\n")
            ...
            cna_window1 = mgs_X_cna[0]
            cna_window2 = mgs_X_cna[1]
            cna_mgs_X_test = clf_cna.mg_scanning(np.array(X_test_cna))
            cna_window1_test = cna_mgs_X_test[0]
            cna_window2_test = cna_mgs_X_test[1]
            train_predict_y_cna = clf_cna.cascade_forest(cna_window1,cna_window2,np.array(y_train_cna))
            if train_predict_y_cna=="no_features":
                feature_flag = "no_features"
                print("drug"+str(drug)+":all feature importances are zeros")
                break
            cascade_all_pred_prob = clf_cna.cascade_forest(cna_window1_test,cna_window2_test)
            pred_proba_cna = np.mean(cascade_all_pred_prob, axis=0)
            # pd.DataFrame(predict_proba_cna).to_csv("two4/"+str(j)+str(drug)+"_cna_predict_proba.csv")
            predictions_cna = clf_cna.levels[np.argmax(pred_proba_cna, axis=1)]
            pd.DataFrame(predictions_cna).to_csv("two4/"+str(j)+str(drug)+"_cna_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("two4/"+str(drug)+"five_results.csv")
        print(five_results)
    ...
    pd.DataFrame(all_drug_acc).to_csv("two4/all_drug_acc.csv")
