...
if __name__ == '__main__':
    ...
    for i in range(25):
        ...
        pd.DataFrame(folds_expr).to_csv("single/folds_expr.csv")
        ...
        pd.DataFrame(folds_cna).to_csv("single/folds_cna.csv")
        ...
        for j in range(5):
            ...
            print("levels:",levels)
            File = open("single/"+str(j)+str(drug)+".txt", "w")
            ...
            pd.DataFrame(predict_proba).to_csv("single/"+str(j)+str(drug)+"_predict_proba.csv")
            predictions = levels[np.argmax(predict_proba, axis=1)]
            pd.DataFrame(predictions).to_csv("single/"+str(j)+str(drug)+"_predictions.csv")
        ...
        if feature_flag is "no_features":
            continue
        pd.DataFrame(five_results).to_csv("single/"+str(drug)+"five_results.csv")
        print(five_results)
        each_drug_acc.append(pd.DataFrame(five_results).iloc[:,1].mean())
        all_drug_acc.append(each_drug_acc)
    pd.DataFrame(all_drug_acc).to_csv("single/all_drug_acc.csv")
