...
class gcForest(object):
    ...
    def cascade_forest(self, X1, X2, y=None):
        ...
        if y is not None:
            ...
            self.n_layer += 1
            if self.n_layer%2==1:
              select_features=self.rf_fs(X_train1, y_train)
              print(len(select_features))
              if len(select_features)==0:
                  return "no_features"
              setattr(self, '_selectfeatures{}'.format(self.n_layer), select_features)
              print("layer1:",X_train1[:,select_features])
              prf_crf_pred_ref = self._cascade_layer(X_train1[:,select_features], y_train)
              accuracy_ref = self._cascade_evaluation(X_test1,X_test2, y_test)
              feat_arr = self._create_feat_arr(X_train2, prf_crf_pred_ref)
              print("layer1 feat_arr:",feat_arr)
            self.n_layer += 1
            if self.n_layer%2==0:
              select_features=self.rf_fs(feat_arr, y_train)
              # print("train:",self.n_layer,select_features)
              setattr(self, '_selectfeatures{}'.format(self.n_layer), select_features)
              print("layer2:",feat_arr[:,select_features])
              prf_crf_pred_layer = self._cascade_layer(feat_arr[:,select_features], y_train)
              accuracy_layer = self._cascade_evaluation(X_test1,X_test2, y_test)
            ...
            if accuracy_layer < accuracy_ref :
                ...
                delattr(self, '_selectfeatures{}'.format(self.n_layer))
                self.n_layer -= 1
            # print("prf_crf_pred_ref:",prf_crf_pred_ref)
        elif y is None:
            at_layer = 1
            select_features=getattr(self, '_selectfeatures{}'.format(at_layer))
            # print("test:",at_layer,select_features)
            if at_layer%2==1:
                prf_crf_pred_ref = self._cascade_layer(X1[:,select_features], layer=at_layer)
            elif at_layer%2==0:
                prf_crf_pred_ref = self._cascade_layer(X2[:,select_features], layer=at_layer)
            while at_layer < getattr(self, 'n_layer'):
                ...
                select_features=getattr(self, '_selectfeatures{}'.format(at_layer))
                # print("test:",at_layer,select_features)
                prf_crf_pred_ref = self._cascade_layer(feat_arr[:,select_features], layer=at_layer)
    ...
    def _cascade_evaluation(self, X_test1,X_test2, y_test):
        ...
        """
        casc_pred_prob = np.mean(self.cascade_forest(X_test1,X_test2), axis=0)
    ...
    def rf_fs(self,X,y):
        ...
        # pd.DataFrame(importances.index[importances.importance>0]).to_csv(str(self.n_layer)+"importances.csv")
        return importances.index[importances.importance>0]
